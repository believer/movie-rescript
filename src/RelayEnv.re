...
let fetchQuery:
  (unit => Js.Promise.t('a)) => ReasonRelay.Network.fetchFunctionPromise =
  (getAccessTokenSilently, operation, variables, _cacheConfig) => {
...
let network = getAccessTokenSilently =>
  ReasonRelay.Network.makePromiseBased(
    ~fetchFunction=fetchQuery(getAccessTokenSilently),
    (),
  );
