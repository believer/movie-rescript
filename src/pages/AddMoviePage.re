    ...
    mutate(
      ~variables={
        ...
        genres:
          movie.genres
          ->Belt.Array.map(({name}) =>
              AddMoviePageMutation_graphql.Types.{
                id: None,
                movie_id: None,
                genre_id: None,
                genre:
        ...
        people:
          people
          ->Belt.Array.keep(({department, job}) =>
              Job.isValid(department, job)
            )
       ...
       | Data(movie) =>
         <div className="flex-1 p-5 mt-8 text-xs bg-gray-200 md:mt-0">
           ...
           <ul>
             {movie.credits.cast
              ->Belt.Array.map(person => {
                  <li> {React.string(person.name)} </li>
                })
              ->React.array}
           </ul>
